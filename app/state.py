...
class State(rx.State):
    ...
    def load_jobs(self):
        try:
            with Session(engine) as session:
                ...
                self.jobs = [job.model_dump() for job in results]
    ...
    def add_job(self):
        ...
        try:
            interval = int(self.new_job_interval)
            if interval < 1:
                raise ValueError
        except ValueError as e:
            logging.exception(f"Invalid interval value: {e}")
            return rx.window_alert("Interval must be a positive integer (seconds).")
        try:
            with Session(engine) as session:
